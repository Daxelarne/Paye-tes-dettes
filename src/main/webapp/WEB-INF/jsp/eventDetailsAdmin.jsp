...
<body>
	<section class="container">
		...
		<hr/>
			<% if (!event.getOwner().equals(user) && event.getParticipants().contains(user)) { %>
			  ...
			  <div class="modal-dialog">
			    <div class="modal-content">
			      <div class="modal-header">
			        <h5 class="modal-title" id="deleteEventLabel">Attention !</h5>
			        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			      </div>
			      <div class="modal-body">
			        <p><b>Vérifiez bien !</b></p>
			        <p>Cette action est <b>définitive</b>, aucun retour en arrière n'est possible ! </p>
			        <p>La totalité des images, commentaires et transactions seront perdues !</p>
			      </div>
			      <div class="modal-footer">
			      	<a class="btn btn-danger" href="/event/delete/<%= event.getId() %>">Je comprends et souhaite supprimer l'événement</a>
			...
			<button class="btn btn-danger" type="button" onclick="confirmDelete()" >Supprimer l'évènement</button>
			
			<a class="btn btn-success" href="/event/manageMembers/<%= event.getId() %>">Gérer les membres...</a>
			<button type="submit" onclick="deployModal()" class="btn btn-primary">Modifier</button>
			<hr>
				 <div class="accordion mb-5" id="accordionExample">
					  <div class="accordion-item">
					    <h2 class="accordion-header" id="headingOne">
					      <button class="accordion-button expanded" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
					        Transactions
					      </button>
					    </h2>
									...
									if(eventDir.listFiles() != null && eventDir.listFiles().length>0) {
										...
										%>
											...
											<span class="artist-collection-photo">
													...
													<a href="/event/deleteImg/${event.getId() }/<%=fileName%>"><img src="/img/remove.png"  width=20 style=" margin-bottom: 16%; transform: translateX(-100%)" title="Supprimer l'image"/></a>
													<% if( fileName.equals(event.getDefaultImg())) { %>
														<a href="/event/favImg/${event.getId() }/<%=fileName%>"><img src="/img/heart_full.png"  width=20 style=" margin-bottom: 16%; transform: translateX(-350%)" title="Retirer des favoris"/></a>
													<% } else { %>
														<a href="/event/favImg/${event.getId() }/<%=fileName%>"><img src="/img/heart.png"  width=20 style=" margin-bottom: 16%; transform: translateX(-350%)" title="Définir comme favoris"/></a>
													<% } %>
											</span>
		      ...
		      <div class="modal-body">
					...
					<div class="mb-3">
						...
						<form:select class="form-control" path="sender" required="true">
							<% for (User participant : event.getParticipants()) {%>
								<form:option value="<%= participant %>" label="<%= participant.getLogin() %>"/>
							<% } %>
						</form:select>
					</div>
	...
	<script>
		...
		<div class="modal fade" id="createEventModal" tabindex="-1" aria-labelledby="createEventModalLabel" aria-hidden="true">
		  <div class="modal-dialog">
		    <div class="modal-content">
		      <div class="modal-header">
		        <h5 class="modal-title" id="createEventModalLabel">Modifier l'évènement</h5>
		        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
		      </div>
		      <form:form method="POST" action="/event/update" modelAttribute="event">
		      <div class="modal-body">
		      
		      		<form:input class="form-control " type="text" path="id" value="${ event.getId() }" hidden="true"/>
		      		
		      		<form:input class="form-control " type="text" path="owner" value="${ event.getOwner().getId() }" hidden="true"/>
					...
					<div class="mb-3">
						...
						<form:select class="form-control" path="visibility">
        					...
        					<form:option value="PRIVATE" label="Privé"/>
        					<form:option value="FRIENDS" label="Amis"/>
						</form:select>
					</div>
					...
					<div class="mb-3">
						...
						<form:input class="form-control" type="date" id="startDate" path="startDate" value="<%= event.getStartDate() %>" required="true" />
					</div>
					
					<div class="mb-3">
						<label for="exampleInputEmail1" class="form-label">Date de fin</label>
						<form:input class="form-control" type="date" id="endDate" path="endDate" value="<%= event.getEndDate() %>" required="true" />
					</div>
